...
const markTex=loader.load('assets/scaleops-mark.png?v=v7-logo4',t=>{t.colorSpace=THREE.SRGBColorSpace;t.anisotropy=Math.min(8,renderer.capabilities.getMaxAnisotropy());t.needsUpdate=true;},undefined,()=>document.documentElement.classList.add('texture-fallback'));
...
function logo(scale=3){const g=new THREE.Group(),aspect=661/433,geo=new THREE.PlaneGeometry(scale*aspect,scale);for(let i=0;i<4;i++){const m=new THREE.Mesh(geo,new THREE.MeshBasicMaterial({map:markTex,transparent:true,opacity:.10+i*.07,depthWrite:false,depthTest:false,toneMapped:false,side:THREE.DoubleSide}));m.position.set(-i*.014,i*.008,-.18+i*.045);m.renderOrder=90+i;g.add(m);}const front=new THREE.Mesh(geo,new THREE.MeshBasicMaterial({map:markTex,transparent:true,alphaTest:.015,depthWrite:false,depthTest:false,toneMapped:false,side:THREE.DoubleSide}));front.position.z=.16;front.renderOrder=100;g.add(front);const h=sprite(C.blue,scale*2.05,.42);h.position.z=-.55;g.add(h);g.renderOrder=100;return g;}
function sphere(r=3){const m=new THREE.MeshPhysicalMaterial({color:0x061832,roughness:.07,metalness:.06,transmission:.26,transparent:true,opacity:.18,clearcoat:1,ior:1.3,thickness:1,emissive:0x031a40,emissiveIntensity:.42,side:THREE.DoubleSide,depthWrite:false});const s=new THREE.Mesh(new THREE.SphereGeometry(r,mobile()?36:60,mobile()?24:40),m);s.renderOrder=1;return s;}
...
function build(){let g=stage(0);g.add(sphere(3));const l0=logo(3.35);l0.position.z=.72;g.add(l0);orbit(g,4.2,.26,C.cyan,.32);orbit(g,4.65,-.16,C.violet,.16);platform(g);
g=stage(1,C.cyan);g.add(sphere(2.5));const l1=logo(2.45);l1.position.z=.64;g.add(l1);nodes(g,10,4.05);orbit(g,4.3,.18,C.cyan,.34);platform(g,-2.9,3.2);
...
g=stage(8,C.blue);g.add(sphere(3));const l8=logo(3.35);l8.position.z=.72;g.add(l8);orbit(g,4.15,.24,C.cyan,.32);orbit(g,4.6,-.15,C.violet,.16);platform(g);}
build();
...
function dom(p,i){sections.forEach((s,k)=>s.classList.toggle('active',k===i));const show=i<=1||i===sections.length-1;if(overlay){overlay.style.setProperty('display','block','important');overlay.style.setProperty('visibility','visible','important');overlay.style.setProperty('opacity',show?'1':'0','important');overlay.style.setProperty('z-index','25','important');overlay.style.setProperty('transform',`translate(-50%,-50%) perspective(1000px) rotateY(${(p-.5)*10}deg) scale(${show?1:.92})`,'important');}const ls=[...document.querySelectorAll('.journey-label span')];ls.forEach((x,k)=>x.classList.toggle('hot',i===k+1||i===Math.min(k+2,6)));}
